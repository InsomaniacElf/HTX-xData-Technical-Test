...
| --- | --- | --- |
| 2c: full TDK decoding | Old run stopped; corrected API path pending | All 12 old jobs confirmed Canceled to avoid idle-GPU spending beyond their six-hour timeout. Recovered all 12 SQLite journals: integrity checks pass, 4,544 successful direct-inference reference rows. These are not the final API-generated dataset. Serial downloads dominated; previous 1.5-4 h estimate was unsupported. Benchmark concurrent downloading and actual API inference before relaunch. |
